...
pub mod events {
    ...
    pub fn tvm_pubkey_confirmed() -> &'static ton_abi::Event {
        crate::once!(ton_abi::Event, || {
            EventBuilder::new("TonPubkeyConfirmed")
                .abi_version(ABI_VERSION)
                .inputs(TvmPubkeyConfirmedEvent::param_type())
                .build()
        })
    }

    pub fn evm_address_confirmed() -> &'static ton_abi::Event {
        crate::once!(ton_abi::Event, || {
            EventBuilder::new("EthAddressConfirmed")
                .abi_version(ABI_VERSION)
                .inputs(EvmAddressConfirmedEvent::param_type())
                .build()
        })
    }
